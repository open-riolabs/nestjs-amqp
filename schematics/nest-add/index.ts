...
export function nestAdd(options: Schema): Rule {
  return chain([
    ...
    options.gateway ? modifyMain(options) : noop(),
    options.skills ? copySkills() : noop(),
...
function createConfigFiles(options: Schema): Rule {
  return (tree: Tree, context: SchematicContext) => {
...
function configLoaderContent(configPath: string): string {
...
function configYamlContent(options: Schema): string {
  const base = `app:
  port: 3000
  host: 0.0.0.0
...
function modifyAppModule(options: Schema): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    try {
      ...
      context.logger.warn(`Failed to edit ${path} (${(e as Error).message}). Add manually:\n${manualSnippet(options)}`);
    }
    return tree;
  };
}

function buildImportLines(options: Schema): string[] {
...
function buildModuleEntries(options: Schema): string[] {
...
function findModuleImportsArray(source: ts.SourceFile): ts.ArrayLiteralExpression | undefined {
...
function findModuleDecoratorObject(source: ts.SourceFile): ts.ObjectLiteralExpression | undefined {
...
function manualSnippet(options: Schema): string {
...
function modifyMain(options: Schema): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    let lastImportEnd = 0;
    source.statements.forEach((s) => { if (ts.isImportDeclaration(s)) lastImportEnd = s.getEnd(); });
...
function copySkills(): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    walk(skillsRoot, '');
    if (copied) context.logger.info(`Copied ${copied} Claude skill file(s) into .claude/skills`);
    else context.logger.warn('No bundled skills found to copy.');
    return tree;
  };
}
